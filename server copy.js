...
app.get('/api/vendors/top', async (req, res) => {
  ...
  const record = await TopVendors.findOne({ weekStart: startOfWeek });
  if (!record) return res.json([]);

  const enriched = await Promise.all(record.vendors.map(async v => {
...
return {
  ...
  location: user.location,
  likes: v.likes,
  kycVerified: kyc?.status === 'verified'  // ✅ Add this too
};
  }));
...
cron.schedule('0 1 * * 0', async () => {
  ...
  await WeeklyLikeLog.deleteMany({ createdAt: { $lt: cutoff } });
  console.log('Old WeeklyLikeLog entries deleted');
});
