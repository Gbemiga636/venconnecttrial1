...
const puzzleWinSchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', unique: true },
  createdAt: { type: Date, default: Date.now }
});
const claimMsgSchema = new mongoose.Schema({
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' },
  message: String,
  fromAdmin: { type: Boolean, default: false },
  createdAt: { type: Date, default: Date.now }
});
const ClaimMessage = mongoose.model('ClaimMessage', claimMsgSchema);
const PuzzleWin = mongoose.model('PuzzleWin', puzzleWinSchema);
...
app.post('/api/puzzle-win', requireAuth, async (req, res) => {
  const userId = req.user.id;
  const { message } = req.body;
  if (!message) return res.status(400).send('Missing message');

  try {
    let win = await PuzzleWin.findOne({ userId });
    if (!win) win = await PuzzleWin.create({ userId });

    await ClaimMessage.create({ userId, message, fromAdmin: false });
    console.log('🎉 Puzzle win & message:', userId, message);

    res.send('Claim recorded and message saved');
  } catch (err) {
    console.error('Puzzle win error:', err);
    res.status(500).send('Server error');
  }
});
app.get('/api/puzzle-claim/messages', requireAuth, async (req, res) => {
  try {
    const messages = await ClaimMessage.find({ userId: req.user.id }).sort('createdAt');
    res.json({ messages });
  } catch (err) {
    console.error('Fetch claim messages error:', err);
    res.status(500).send('Server error');
  }
});
app.get('/api/admin/claims', requireAuth, async (req, res) => {
  if (!req.user.isAdmin) return res.status(403).send('Admins only');

  const wins = await PuzzleWin.find().lean();
  const data = await Promise.all(wins.map(async w => {
    const msgs = await ClaimMessage.find({ userId: w.userId }).sort('createdAt').lean();
    const usr = await User.findById(w.userId);
    return { user: { id: usr._id, name: usr.name, email: usr.email }, messages: msgs };
  }));

  res.json(data);
});

app.post('/api/admin/claims/:userId/reply', requireAuth, async (req, res) => {
  if (!req.user.isAdmin) return res.status(403).send('Admins only');
  const { text } = req.body;
  if (!text) return res.status(400).send('Missing text');

  const msg = await ClaimMessage.create({ userId: req.params.userId, message: text, fromAdmin: true });
